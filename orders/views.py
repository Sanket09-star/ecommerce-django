...
def payments(request):
    ...
    #Moeve the cart items to order product table
    ...
    for item in cart_items:
        ...
        #Set the variations for the order product
    ...
    #Clear the cart
    CartItem.objects.filter(user=request.user).delete() #Delete all the cart items of the user 

    #Send order confirmation email to the customer
    mail_subject = 'Thank you for your order!'
    message = render_to_string('orders/order_recieved_email.html', {'user': request.user,'order': order})
    to_email = request.user.email #get the email of the user
    send_email = EmailMessage(mail_subject, message, to=[to_email])
    send_email.send()

    #Send order number and transaction id back to sendData method via JsonResponse / sendData function(payments.html)
    data = {
...
def place_order(request, total=0, quantity=0):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            data.first_name = form.cleaned_data['first_name'] #cleaned_data is used to get the data from the form
            ...
            current_date = d.strftime("%Y%m%d") #20250304
